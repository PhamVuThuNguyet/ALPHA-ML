...
import numpy as np
import pandas as pd
from flask import Flask, session, render_template, make_response, jsonify, request, send_from_directory
from flask import escape
from flask_expects_json import expects_json
from flask_limiter import Limiter
from flask_limiter.util import get_remote_address
from helper import vector_cos5, isValid, isInDB, findFeatures, findSymptom, findDisease, syInData
from settings import APP_CONFIG, PORT
...
def get_model():
    ...
    with open(model_file, 'rb') as f:
        # mnb = joblib.load(f)
        mnb = pkl.load(f)
        data = pd.read_csv("../data/all-files-for-ml/all_x_KB.csv")
...
def predict():
    ...
    sample = np.zeros((len(LABELS),), dtype=np.int64)
    sampe = sample.tolist()
