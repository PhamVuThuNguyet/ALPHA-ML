...
axiosClient.interceptors.request.use(
    async (config) => {
        console.log("axiosClient token: " +localStorage.getItem("access_tk"));
...
const fetchPredictions = (lng, symptoms) => {
    console.log('/api/predict?lang=' + lng, { "symptoms": symptoms })
    return fetcher.post('/api/predict?lang=' + lng, { "symptoms": symptoms }).then(res => {
        console.log("Predict data: " + res.data);
...
class Index extends Component {
    ...
    createPredicts = () => {
        ...
        if(resultArray.length >= 1) {
            const postData = {
                ...
                symtomps: resultArray[0].sy.toString(),
                predicted_results: resultArray
            }
        
            axiosClient.post('/diagnosis', postData).then(res => {
                console.log(res);
                window.location.replace(`http://127.0.0.1:3000/result#${res.data._id}`);
            })
        }
    }
    ...
    addToSearch = (suggestion) => {
        ...
        let vals = [...value, suggestion].map((v) => {
            return v.value
        })
    ...
    giveFeedbackNewDisease = () => {
        ...
        let vals = newDiseaseSymptoms.map((v) => {
            return v.label
        })
    ...
    handleClap = (disease) => {
        ...
        let vals = this.state.value.map((v) => {
            return v.label
        })
    ...
    componentWillMount() {
        ...
        if (defaultValues.length != 0) {
            ...
            if (defaultValues.every(function (i) {
                return typeof i === "string"
            })) {
                vals = defaultValues;
            } else {
    ...
    render() {
        ...
        let symptoms = this.state.value.map((v) => {
            return v.label
        }).join(', ');
        let transl = this.state.language === 'en' ? 'RELATED: ' : 'DAZUGEHÖRIG: ';
        let suggestionVals = this.state.suggestions.length >= 1 ? this.state.suggestions.map((v) => {
            return v.label
        }) : [];
                                                            ...
                                                            <div>
                                                                ...
                                                                {this.state.suggestions.map((s, i) => {
                                                                    if (this.state.suggestions.length - 1 === i) {
                                                                        return (
                                                                            <div
                                                                                className={classNames(this.props.classes.inlineBlock)}
                                                                                key={"div" + s.label}>
                                                                                <a key={s.label}
                                                                                    onClick={() => this.addToSearch(s)}
                                                                                    className="mr-1">{s.label}</a>
                                                                            </div>
                                                                        )
                                                                    } else {
                                                                        return (
                                                                            <div
                                                                                ...
                                                                                key={"div" + s.label}>
                                                                                <a key={s.label}
                                                                                    onClick={() => this.addToSearch(s)}> {s.label} </a>
                                                                                <small className="mr-1"
                                                                                    key={"small" + s.label}> • </small>
                                                                            </div>
                                                                        )
                                                                    }
                                                                })}
                                        ...
                                        <div>
                                            ...
                                            <div className="">
                                                {this.state.results.map((pred) => {
                                                    return (
                                                        <div className={this.props.classes.cardListItem}
                                                            key={"li" + pred.disease}>
                                                            <Card className={this.props.classes.card}>
                                                                ...
                                                                <CardActions className={this.props.classes.actions}
                                                                    ...
                                                                    <IconButton aria-label="Looking for this?"
                                                                        onClick={() => this.handleClap(pred.disease)}>
                                                                        <FavoriteIcon />
                                                                    </IconButton>
                                                                    <IconButton
                                                                ...
                                                                <Collapse in={this.shouldExpandPanel(pred.disease)}
                                                                            ...
                                                                            <NamespacesConsumer>
                                                                                {
                                                                                    t => (<Chip
                                                                                        ...
                                                                                        onClick={this.handleRequestDelete(pred.disease, this.state.value.map((v) => {
                                                                                            return v.label
                                                                                        }))}
                                        ...
                                        <Grid container spacing={8}>
                                            ...
                                            <Grid item xs={4}>
                                                ...
                                                <ul className={classes.noBullets}>
                                                    <li><a onClick={() => this.changeLanguage('en')}> <FlagEN width={20}
                                                        height={20} /> en</a>
                                                    </li>
                                                </ul>
                                            </Grid>
